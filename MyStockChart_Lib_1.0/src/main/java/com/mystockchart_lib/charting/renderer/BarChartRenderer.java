...
import com.mystockchart_lib.charting.buffer.BarBuffer;
...
import com.mystockchart_lib.charting.interfaces.dataprovider.BarDataProvider;
...
public class BarChartRenderer extends DataRenderer {
    ...
    protected void drawDataSet(Canvas c, IBarDataSet dataSet, int index) {
        ...
        mBarBorderPaint.setStrokeWidth(dataSet.getBarBorderWidth() * 2);
...
//                    }
                    mRenderPaint.setColor(dataSet.getColors().get(i));
                    mBarBorderPaint.setColor(dataSet.getColor(j / 4));

                    if (drawBorder && dataSet.getEntryForIndex(i).getIsUp()) {
            ...
            for (int j = 0; j < buffer.size(); j += 4) {
              ...
              /*  c.drawRect(buffer.buffer[j], buffer.buffer[j + 1], buffer.buffer[j + 2] - (buffer.buffer[j + 2] - buffer.buffer[j]) / 3,
                        buffer.buffer[j + 3], mRenderPaint);*/
                if (drawBorder&& dataSet.getEntryForIndex(j / 4).getIsUp()) {
                    c.drawRect(buffer.buffer[j], buffer.buffer[j + 1], buffer.buffer[j + 2],
                            buffer.buffer[j + 3], mBarBorderPaint);
                } else {
                    c.drawRect(buffer.buffer[j], buffer.buffer[j + 1], buffer.buffer[j + 2],
