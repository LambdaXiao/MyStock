...
import com.mystockchart_lib.charting.utils.ColorTemplate;
...
public class CandleStickChartRenderer extends LineScatterCandleRadarRenderer {
    ...
    private float[] mShadowBuffers1 = new float[4];
    private float[] mShadowBuffers2 = new float[4];
    ...
    protected void drawDataSet(Canvas c, ICandleDataSet dataSet) {
        ...
        for (int j = minx,
            ...
            if (showCandleBar) {
                ...
                mShadowBuffers1[0] = xIndex;
                mShadowBuffers1[2] = xIndex;
                mShadowBuffers2[0] = xIndex;
                mShadowBuffers2[2] = xIndex;

                if (open > close) {
                    mShadowBuffers1[1] = high>open?high * phaseY:open * phaseY;
                    mShadowBuffers1[3] = open * phaseY;
                    mShadowBuffers2[1] = low<close?low * phaseY:close * phaseY;
                    mShadowBuffers2[3] = close * phaseY;
                } else if (open < close) {
                    mShadowBuffers1[1] = high>close?high * phaseY:close * phaseY;
                    mShadowBuffers1[3] = close * phaseY;
                    mShadowBuffers2[1] = low<open?low * phaseY:open * phaseY;
                    mShadowBuffers2[3] = open * phaseY;
                } else {
                    mShadowBuffers1[1] = high>open?high * phaseY:open * phaseY;
                    mShadowBuffers1[3] = open * phaseY;
                    mShadowBuffers2[1] = low<open?low * phaseY:open * phaseY;
                    mShadowBuffers2[3] = mShadowBuffers1[3];
                }

                trans.pointValuesToPixel(mShadowBuffers1);
                trans.pointValuesToPixel(mShadowBuffers2);
                ...
                c.drawLines(mShadowBuffers1, mRenderPaint);
                c.drawLines(mShadowBuffers2, mRenderPaint);
