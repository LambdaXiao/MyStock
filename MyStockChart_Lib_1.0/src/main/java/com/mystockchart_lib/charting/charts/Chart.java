...
import com.mystockchart_lib.charting.animation.ChartAnimator;
import com.mystockchart_lib.charting.animation.Easing;
import com.mystockchart_lib.charting.animation.EasingFunction;
...
import com.mystockchart_lib.charting.formatter.DefaultValueFormatter;
import com.mystockchart_lib.charting.formatter.ValueFormatter;
import com.mystockchart_lib.charting.highlight.ChartHighlighter;
import com.mystockchart_lib.charting.highlight.Highlight;
import com.mystockchart_lib.charting.interfaces.dataprovider.ChartInterface;
import com.mystockchart_lib.charting.interfaces.datasets.IDataSet;
import com.mystockchart_lib.charting.listener.ChartTouchListener;
import com.mystockchart_lib.charting.listener.OnChartGestureListener;
import com.mystockchart_lib.charting.listener.OnChartValueSelectedListener;
import com.mystockchart_lib.charting.renderer.DataRenderer;
import com.mystockchart_lib.charting.renderer.LegendRenderer;
...
public abstract class Chart<T extends ChartData<? extends IDataSet<? extends Entry>>> extends
    ...
    public static final String LOG_TAG = "MyChart";
        ...
        if(mIsDescriptionCustom){
            ...
            if (mDescriptionPosition == null) {
                for(int i=0;i<mDescriptionLabels.length;i++){
                    ...
                    xpos = xpos + Utils.calcTextWidth(mDescPaint, mDescriptionLabels[i]);
                    c.drawText(mDescriptionLabels[i], xpos + 10,
                ...
                for(int i=0;i<mDescriptionLabels.length;i++) {
                    mDescPaint.setColor(mDescriptionColors[i]);
                    xpos = xpos + Utils.calcTextWidth(mDescPaint, mDescriptionLabels[i]);
                    c.drawText(mDescriptionLabels[i], xpos+mDescriptionPosition.x, mDescriptionPosition.y, mDescPaint);
                }
            }
        }else if (!"".equals(mDescription)) {

            if (mDescriptionPosition == null) {
                c.drawText(mDescription, mViewPortHandler.contentLeft() + Utils.calcTextWidth(mDescPaint, mDescription) + 10,
                        mViewPortHandler.contentTop() + Utils.calcTextHeight(mDescPaint, mDescription) + 10, mDescPaint);
            } else {
    ...
    public boolean saveToGallery(String fileName, int quality) {
        return saveToGallery(fileName, "", "MyChart-Library Save", CompressFormat.JPEG, quality);
    }
